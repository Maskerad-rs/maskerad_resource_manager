...
use properties::property_manager_error::{PropertyManagerError, PropertyManagerResult};
...
impl PropertyRegistry {
    ...
    pub fn get_mesh_of<I: AsRef<str>>(&self, gameobject_id: I) -> PropertyManagerResult<&Mesh> {
        match self.mesh_registry.get(gameobject_id.as_ref()) {
            Some(mesh) => {
                Ok(mesh)
            },
            None => {
                Err(PropertyManagerError::MeshError(format!("Could not find the mesh linked to the gameobject {} in the mesh registry !", gameobject_id.as_ref())))
            },
        }
    }

    pub fn get_transform_of<I: AsRef<str>>(&self, gameobject_id: I) -> PropertyManagerResult<&Transform> {
        match self.transform_registry.get(gameobject_id.as_ref()) {
            Some(transform) => {
                Ok(transform)
            },
            None => {
                Err(PropertyManagerError::TransformError(format!("Could not find the transform linked to the gameobject {} in the transform registry !", gameobject_id.as_ref())))
            }
        }
    }
